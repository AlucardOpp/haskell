...
-- (+) - функция, используемая в качестве применяемой функции в zipWith для сложения элементов двух списков
...
oddEvenSwap [] = []
oddEvenSwap [x] = [x]
oddEvenSwap (x:y:xs) = y:x:oddEvenSwap xs
...
-- | (n == 1) = 2 - это случай, когда n равно 1, тогда 2n равно 2
-- | (even n) = w * w - это случай, когда n четное, тогда 2n равно (2k)2
-- | otherwise = (w * w) + (w * w) - это случай, когда n нечетное, тогда 2n равно 2 * (2k)2
...
substitute c d xs = map (\x -> if x == c then d else x) xs
